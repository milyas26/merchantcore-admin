  ...
  if (error) {
    return (
      <Alert variant="destructive">
        <AlertDescription>
          {(error as any)?.error?.message || "Gagal memuat promosi"}
        </AlertDescription>
      </Alert>
    );
  }
            ...
            <TableBody>
              {isLoading ? (
                ...
                promotions.map((promo, idx) => {
                  ...
                  const productCount =
                    p._count?.promotionProducts ??
                    p.promotionProducts?.length ??
                    0;
                  const isExpired = p.endsAt && new Date(p.endsAt) < new Date();
                  const isUpcoming =
                    p.startsAt && new Date(p.startsAt) > new Date();
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            "text-[10px] h-5",
                            typeBadgeClass[p.type] || "bg-gray-100",
                          )}
                        >
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {p.startsAt
                          ? new Date(p.startsAt).toLocaleDateString("id-ID")
                          : "—"}
                        {" – "}
                        {p.endsAt
                          ? new Date(p.endsAt).toLocaleDateString("id-ID")
                          : "Tanpa batas"}
                      </TableCell>
                      <TableCell className="text-center">
                        {!p.isActive ? (
                          ...
                          <Badge
                            variant="secondary"
                            className="bg-red-100 text-red-700 text-[10px] h-5 border-0"
                          >
                            Berakhir
                          </Badge>
                        ) : isUpcoming ? (
                          <Badge
                            variant="secondary"
                            className="bg-purple-100 text-purple-700 text-[10px] h-5 border-0"
                          >
                            Terjadwal
                          </Badge>
                        ) : (
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                "{p.title}" akan dihapus permanen. Tindakan ini
            ...
            <div className="flex items-center gap-1">
              ...
              {getPageNumbers().map((pg, i) =>
                pg === "..." ? (
                  ...
                  <Button
                    key={pg}
                    variant={pagination.page === pg ? "default" : "outline"}
                    size="icon-sm"
                    className="text-xs"
